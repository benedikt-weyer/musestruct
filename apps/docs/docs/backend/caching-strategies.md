...
The current cache-related behavior is concentrated in the backend and falls into four categories:
...
- Local cover image responses served from `/api/stream/local/cover/{path}` use `Cache-Control: public, max-age=86400`.
